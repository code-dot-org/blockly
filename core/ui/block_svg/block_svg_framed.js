...
Blockly.BlockSvgFramed.prototype.initChildren = function() {
  ...
  this.frameText_.appendChild(
    document.createTextNode(Blockly.Msg.FUNCTION_HEADER)
  );
...
Blockly.BlockSvgFramed.prototype.renderDraw_ = function(iconWidth, inputRows) {
  Blockly.BlockSvgFramed.superClass_.renderDraw_.call(
    this,
    iconWidth,
    inputRows
  );
  ...
  if (!this.frameText_.getAttribute('y')) {
    ...
    var textHeight = Math.abs(
      this.frameText_.getBoundingClientRect().top -
        this.svgPathDark_.getBoundingClientRect().top
    );
    this.frameText_.setAttribute(
      'y',
      -(FRAME_MARGIN_TOP + (FRAME_HEADER_HEIGHT - textHeight) / 2)
    );
  }
};
