...
function test_field_scroll_listener() {
  ...
  assertEquals(
    0,
    blockSpace.events.getListeners(
...
function test_dropdown_options() {
  ...
  assert(
    goog.array.equals(
      ['Foo 123', 'foo', 'abc', 'abc', 'zzz', 'zzz'],
      goog.array.flatten(dropdown.getOptions())
    )
  );
...
function test_image_dropdown_menu_button() {
  ...
  var blockSpace = Blockly.mainBlockSpace;
  var imgDropdown = new Blockly.FieldImageDropdown(
    [['#', 'foo'], ['#', 'bar']],
    200,
    200,
    [{text: 'TestButton', action: function() {}}]
  );
...
function test_clampedNumberValidator() {
  ...
  var withNegativeLowerBound = Blockly.FieldTextInput.clampedNumberValidator(
    -0.5
  );
  var withUpperBound = Blockly.FieldTextInput.clampedNumberValidator(
    undefined,
    4.5
  );
